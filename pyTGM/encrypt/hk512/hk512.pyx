...
cdef extern from "hk512.hpp":
    cpp_string encode_ "encode" (const cpp_string& input, const cpp_string& key)
    cpp_string decode_ "decode" (const cpp_string& data, const cpp_string& key)

cpdef str encode(str input_text, str key):
...
cpdef str decode(str data, str key):
